...
window.onload = function(): void {
    hand = <HTMLDivElement>document.getElementById("hand");
...
function addToHand(): void {
  ...
  for (let i: number = 0; i < 5; i++) { 
    ...
    let deck: string = spielkarten[Math.floor(Math.random() * (spielkarten.length - 1))];
    let handKarten: HTMLDivElement = <HTMLDivElement>document.createElement("div");
...
function addToAblage(): void {
    
    let deck: string = spielkarten[Math.floor(Math.random() * (spielkarten.length - 1))];
    let ablageStapel: HTMLDivElement = <HTMLDivElement>document.createElement("div");
