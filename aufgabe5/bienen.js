...
(function (L4_Canvas) {
    ...
    let crc2;
    let image; // image als ImageData deklarieren  
    let x = []; //x-Koordinten
    let y = []; //y-Koordinaten
    let n = 10; //Anzahl Bienenschwarm
    function init(_event) {
        ...
        for (var i = 0; i < 15; i++) {
            ...
            let _x = Math.floor(Math.random() * (400 + 100)) - 100; //Blumen werden auf einer Breite von 420px generiert. Vom rechten Rand zum linken Rand.
    ...
    function animate() {
        ...
        for (let i = 0; i < n; i++) {
            x[i] += Math.random() * 5 - 4; //Bewegungsfreiraum von -5 und 5. Zug nach -4.
            y[i] += Math.random() * 9 - 4.5; //Bewegungsfreiraum von -9 und 9. Zug nach -4.5.
            ...
            if (y[i] > 645)
                y[i] = 5;
            drawBees(x[i], y[i]); //Bienen mit den neuen Koordinatendaten i zeichnen
        }
